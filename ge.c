...
void ge_add(ge p3, const ge p1, const ge p2)
{
    ...
    */
    /*   #: Instruction number as mentioned in the paper */
             fe12_mul(t0, x1, x2); // |t0| ≤ 1.01 * 2^22
             fe12_mul(t1, y1, y2); // |t1| ≤ 1.01 * 2^22
             fe12_mul(t2, z1, z2); // |t2| ≤ 1.01 * 2^22
             fe12_add(t3, x1, y1); // |t3| ≤ 1.01 * 2^23
    /*  5 */ fe12_add(t4, x2, y2); // |t4| ≤ 1.01 * 2^23
             fe12_copy(t5, t3); fe12_mul(t3, t5, t4); // |t3| ≤ 1.01 * 2^21
             fe12_add(t4, t0, t1); // |t4| ≤ 1.01 * 2^23
             fe12_sub(t3, t3, t4); // |t3| ≤ 1.01 * (2^23 + 2^21)
             fe12_add(t4, y1, z1); // |t4| ≤ 1.01 * (2^22 + 2^21)
    /* 10 */ fe12_add(x3, y2, z2); // |x3| ≤ 1.01 * (2^22 + 2^21)
             fe12_copy(t5, t4); fe12_mul(t4, t5, x3); // |t4| ≤ 1.01 * 2^21
             fe12_add(x3, t1, t2); // |x3| ≤ 1.01 * 2^23
             fe12_sub(t4, t4, x3); // |t4| ≤ 1.01 * 2^23
             fe12_add(x3, x1, z1); // |x3| ≤ 1.01 * (2^22 + 2^21)
    /* 15 */ fe12_add(y3, x2, z2); // |y3| ≤ 1.01 * (2^22 + 2^21)
             fe12_copy(t5, x3); fe12_mul(x3, t5, y3); // |x3| ≤ 1.01 * 2^21
             fe12_add(y3, t0, t2); // |y3| ≤ 1.01 * 2^23
             fe12_sub(y3, x3, y3); // |y3| ≤ 1.01 * 2^23
             fe12_mul_b(z3, t2);   // |z3| ≤ 1.01 * 2^21
    /* 20 */ fe12_sub(x3, y3, z3); // |x3| ≤ 1.01 * (2^23 + 2^21)
             fe12_add(z3, x3, x3); // |z3| ≤ 1.01 * (2^24 + 2^22)
             fe12_add(x3, x3, z3); // |x3| ≤ 1.01 * (2^24 + 2^23 + 2^22 + 2^21)
             fe12_copy(t5, t1); fe12_sub(z3, t5, x3); // |z3| ≤ 1.01 * (2^25 + 2^21)
             fe12_add(x3, t1, x3); // |x3| ≤ 1.01 * (2^25 + 2^21)
    /* 25 */ fe12_mul_b(y3, y3);   // |y3| ≤ 1.01 * 2^21
             fe12_add(t1, t2, t2); // |t1| ≤ 1.01 * 2^23
             fe12_add(t2, t1, t2); // |t2| ≤ 1.01 * (2^23 + 2^22)
             fe12_sub(y3, y3, t2); // |y3| ≤ 1.01 * (2^23 + 2^22 + 2^21)
             fe12_sub(y3, y3, t0); // |y3| ≤ 1.01 * (2^24 + 2^21)
    /* 30 */ fe12_add(t1, y3, y3); // |t1| ≤ 1.01 * (2^25 + 2^22)
             fe12_add(y3, t1, y3); // |y3| ≤ 1.01 * (2^25 + 2^24 + 2^22 + 2^21)
             fe12_add(t1, t0, t0); // |t1| ≤ 1.01 * 2^23
             fe12_add(t0, t1, t0); // |t0| ≤ 1.01 * (2^23 + 2^22)
             fe12_sub(t0, t0, t2); // |t0| ≤ 1.01 * (2^24 + 2^23)
